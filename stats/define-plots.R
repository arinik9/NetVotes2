		...
		is.y.axis.log.scaled=FALSE)
{
	...
	if(dashed.line.desc!="NA" || (!is.na(desc1) && !is.na(desc2)) ) has.legend=TRUE
		...
		filepath.filt, filepath.orig.graph.str=NA, filepath.filt.graph.str=NA, method)
{
	...
	my.plot(plot.filename=output.filename,
			x1=x, x2=x, y1=y1, y2=y2, xlabel="instaces (decr ordered by #clusters based on unfiltered graphs)", ylabel="#cluster",
